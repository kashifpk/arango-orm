...
from arango_orm_v1.fields import (  # pylint: disable=E0611
    String, Integer, Boolean)
from arango_orm_v1.database import Database  # pylint: disable=E0401
# from arango_orm.collections import Collection, Relation  # pylint: disable=E
from arango_orm_v1.collections import Collection, Relation
from arango_orm_v1.graph import Graph, GraphConnection  # pylint: disable=E0401
...
class SpecializesIn(Relation):
    ...
    # TODO: handle _key required or auto-generated
...
""".format(sys.argv[0])
    ...
    action = sys.argv[1]
    db_name = sys.argv[2] if len(sys.argv) > 2 else 'arango_orm_v1_test'
    username = sys.argv[3] if len(sys.argv) > 3 else 'kashif'
    password = sys.argv[4] if len(sys.argv) > 4 else 'compulife'
    host = sys.argv[5] if len(sys.argv) > 5 else 'http://127.0.0.1:8529'
